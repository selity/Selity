...
sub newDebug{
	my $debug	= shift || '';
	push(@{Selity::Debug->new()->{logLevels}}, Selity::Debug->new()->{lastLog});
	Selity::Debug->new()->{lastLog} = Log::Message->new( private => 1 );
	Selity::Debug->new()->{log}->{$debug} = Selity::Debug->new()->{lastLog};
}

sub endDebug{
	my $self = Selity::Debug->new();
...
sub silent{
	Selity::Debug->new()->{silent} = shift || 0;
	debug("Enter silent mode") if Selity::Debug->new()->{silent};
}
...
sub verbose{
	...
	Selity::Debug->new()->{verbose}= $verbose;
}

sub debug{
	return unless Selity::Debug->new()->{verbose};
	my $message		= shift || '';
	my $self		= Selity::Debug->new();
...
END{
	...
	my $self	= Selity::Debug->new();
	my $logdir	= $main::selityConfig{LOG_DIR} || $main::defaultConf{LOG_DIR} || '/tmp';
	my $msg;
