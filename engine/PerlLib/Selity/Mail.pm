...
sub errmsg{
	...
	my @parts = split('@', $main::selityConfig{'DEFAULT_ADMIN_ADDRESS'});
	my $dmn = pop(@parts);
	...
	my $server_name	= $main::selityConfig{'SERVER_HOSTNAME'};
	my $server_ip	= $main::selityConfig{'BASE_SERVER_IP'};
	my $fname		= (caller(1))[3];
	...
	$out->build(
		...
		Data		=> $msg_data,
		'X-Mailer'	=> "Selity $main::selityConfig{'Version'} Automatic Error Messenger"
	);
