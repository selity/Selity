...
@ISA = ('Selity::IP::abstractIP');
use Selity::IP::abstractIP;
...
sub attachIpToNetCard{
	...
	my $rs = execute("$main::selityConfig{'CMD_IFCONFIG'} $card inet6 add $ip/64", \$stdout, \$stderr);
...
sub detachIpFromNetCard{
	...
	my $rs = execute("$main::selityConfig{'CMD_IFCONFIG'} $card inet6 del $ip/64", \$stdout, \$stderr);
	debug("$stdout")if $stdout;
	error("$stderr")if $stderr;
