...
sub loadIPs{
	...
	unless ($self->{_loaded}){
		$rs = execute("$main::selityConfig{'CMD_IFCONFIG'}", \$netCardUp, \$stderr);
		debug("$netCardUp") if ($netCardUp);
		error("$stderr") if ($stderr);
		return $rs if $rs;

		$rs = execute("$main::selityConfig{'CMD_IFCONFIG'} -a", \$configuredIPs, \$stderr);
		debug("$configuredIPs") if ($configuredIPs);
		error("$stderr") if ($stderr);
		return $rs if $rs;

		for('IPv4', 'IPv6'){
			my $file	= "Selity/IP/$_.pm";
			my $class	= "Selity::IP::$_";
			require $file;
			$self->{$_} = $class->new();
