...
sub delDmn{
	...
	$rs |= Selity::Dir->new(dirname => $data->{HOME_DIR})->remove() if -d $data->{HOME_DIR};
...
sub addHtaccess{
	...
	my $bTag	=	"\t\t### START SELITY PROTECTION ###\n";
	my $eTag	=	"\t\t### END SELITY PROTECTION ###\n";
...
sub del_tmp{
	...
	if(-d "/var/www/selity/gui/data/sessions"){
		my $cmd = '[ -x /usr/lib/php5/maxlifetime ] && find /var/www/selity/gui/data/sessions/ -type f -cmin +$(/usr/lib/php5/maxlifetime) -delete';
		$rs |= execute($cmd, \$stdout, \$stderr);
		debug($stdout) if $stdout;
		error($stderr) if $stderr;
		error("Error while executing $cmd.\nReturned value is $rs") if !$stderr && $rs;
