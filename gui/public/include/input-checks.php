...
function selity_check_local_part($email, $num = 50) {
	if (strlen($email) > $num)
		return false;
...
/**
 *
 * @function selity_limit_check
 * @description Function for checking Selity limits.
 * @param string $data Selity 'limit' field data (by default valids are numbers greater equal 0)
 ...
 * @example selity_limit_check($_POST['domains_limit'], null)
 * @example selity_limit_check($_POST['ftp_accounts_limit'])
 */
function selity_limit_check($data, $extra = -1) {
 ...
 * 					for usage in selity_* functions
 ...
 *
 * @function chk_dname
 * @description Function for checking Selity domains syntax. Here domains are
 * 					limited to {dname}.{ext} parts
 * @param String $dname selity domain data
 * @param int $num number of max. chars
 * @return boolean false	incorrect syntax
 * 										true	correct syntax
 */
 ...
 * Description:
 *
 * Function for checking Selity subdomain syntax. Here subdomains are
