...
function generate_reseller_user_props ($reseller_id) {
	...
	while ($data = $res->FetchRow()) {
		...
		list ($tmpval1,
			...
			$tmpval7,
			$tmpval8
			) = generate_user_traffic($admin_id);
...
function get_user_props ($admin_id){
	$sql = Database::getInstance();
	...
	$cnt = get_user_running_props_cnt($admin_id);

	$sub_current = $cnt['sub_cnt'];
	$sub_max = $data['max_sub'];

	$als_current = $cnt['als_cnt'];
	$als_max = $data['max_als'];

	$mail_current = $cnt['mail_cnt'];
	$mail_max = $data['max_mail'];

	$ftp_current = $cnt['ftp_cnt'];
	$ftp_max = $data['max_ftp'];

	$sql_db_current = $cnt['sqld_cnt'];
	$sql_db_max = $data['max_sqldb'];

	$sql_user_current = $cnt['sqlu_cnt'];
	$sql_user_max = $data['max_sqlu'];

	$traff_max = $data['max_traff'];

	$disk_max = $data['max_disk'];
...
function check_ruser_data (&$tpl, $NoPass) {
	...
	if ('_no_' == $NoPass) {
		...
		$user_add_error = tr('The password must be at least %s long and contain letters and numbers to be valid.', Config::get('PASSWD_CHARS'));
	  } else {
		$user_add_error = tr('Password data is shorter than %s signs or includes not permitted signs!', Config::get('PASSWD_CHARS'));
	  }
		}
	}
	...
	else if(!selity_limit_check($customer_id)){
	...
	}else if(!selity_limit_check($zip)){
...
// @author sci2tech
function selity_domain_exists($domain_name){
	...
	$query = '
		...
		WHERE
			`alias_name` = ?
	';
	$rs = mysql::getInstance()->doQuery($query, $domain_name);
	...
	foreach($domain_labels as $label) {
		$dmn = $label.'.'.$dmn;
		$rs =  mysql::getInstance()->doQuery($query, trim($dmn, '.'));
		if ($rs->cnt > 0) return true;
	}
	return false;
}

function gen_manage_user_query (&$search_query, &$count_query,
	$start_index,
	$rows_per_page,
	$search_for = '',
	$search_common = '',
	...
	$add_query = is_null($reseller_id) ? '' : " AND `created_by` = '$reseller_id'";

	if ($search_for != '') {
		if ($search_common === 'domain_name') {
			$add_query .= " AND `admin_name` rlike '" . addslashes($search_for) . "' ";
		} else if ($search_common === 'customer_id') {
			$add_query .= " AND `customer_id` rlike '" . addslashes($search_for) . "' ";
		} else if ($search_common === 'lname') {
			$add_query .= " AND (`lname` rlike '" . addslashes($search_for) . "' or fname rlike '" . addslashes($search_for) . "') ";
		} else if ($search_common === 'firm') {
			$add_query .= " AND `firm` rlike '" . addslashes($search_for) . "' ";
		} else if ($search_common === 'city') {
			$add_query .= " AND `city` rlike '" . addslashes($search_for) . "' ";
		} else if ($search_common === 'country') {
			$add_query .= " AND `country` rlike '" . addslashes($search_for) . "' ";
		}
	}
...
function gen_user_search_options (&$tpl, $search_for, $search_common, $search_status) {
	...
	if ($search_common === 'domain_name' || $search_common === 'n/a') {
	...
	if ($search_status === 'all' || $search_status === 'n/a') {
...
function gen_def_language(&$tpl, &$sql, &$user_def_language) {
	...
	foreach($languageList as $lang){
		...
		$tpl->assign(array(
			'LANG_VALUE'	=> $lang,
			'LANG_SELECTED' => $_SESSION['user_def_lang'] == $lang ? 'selected' : '',
			'LANG_NAME'		=> gettext('Localised language')
		));
		$tpl->parse('DEF_LANGUAGE', '.def_language');
	}
}

function gen_domain_details(&$tpl, &$sql, $admin_id) {
	...
	if (isset($_SESSION['details']) and $_SESSION['details'] == 'hide') {
		...
		$alias_query = '
			...
			ORDER BY
				`alias_id` DESC
		';
		$alias_rs = exec_query($sql, $alias_query, array($admin_id));
...
';
	...
	$dmn_current = $data['current_usr_cnt'];
	$dmn_max = $data['max_usr'];
...
function send_order_emails($admin_id, $domain_name, $ufname, $ulname, $uemail, $order_id) {
	...
	$headers = "From: ". $FROM . "\n";
	$headers .= "MIME-Version: 1.0\n" . "Content-Type: text/plain; charset=utf-8\n" . "Content-Transfer-Encoding: 8bit\n" . "X-Mailer: Selity " . Config::get('Version') . " Service Mailer";
...
function client_mail_add_default_accounts($admin_id, $user_email, $dmn_part, $dmn_type = 'alias', $sub_id = 0){
	$sql = Database::getInstance();

	if (Config::get('CREATE_DEFAULT_EMAIL_ADDRESSES')) {

		$forward_type = 'alias_forward';
...
function reseller_limits_checkreseller_limits_check(&$sql, &$err_msg, $reseller_id, $hpid, $newprops = "") {
	$error = false;
...
function reseller_limits_check(&$sql, &$err_msg, $reseller_id, $hpid, $newprops = '') {
	...
	if ($dmn_max != 0) {
		if ($dmn_current + 1 > $dmn_max) {
			set_page_message(tr('You have reached your user limit.<br />You cannot add more users!'));
			$error = true;
		}
	}
