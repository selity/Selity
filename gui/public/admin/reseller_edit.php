...
$tpl->assign(
	array(
		'TR_ADMIN_EDIT_RESELLER_PAGE_TITLE' => tr('Selity - Admin/Manage users/Edit Reseller'),
...
function check_user_data() {
	...
	if (!selity_limit_check($_POST['nreseller_max_domain_cnt'], null)) {
		set_page_message(tr("Incorrect domains limit!"));
		return false;
	}

	if (!selity_limit_check($_POST['nreseller_max_subdomain_cnt'], -1)) {
		set_page_message(tr("Incorrect subdomains limit!"));
		return false;
	}

	if (!selity_limit_check($_POST['nreseller_max_alias_cnt'], -1)) {
		set_page_message(tr('Incorrect aliases limit!'));
		return false;
	}

	if (!selity_limit_check($_POST['nreseller_max_ftp_cnt'], -1)) {
		set_page_message(tr('Incorrect FTP accounts limit!'));
		return false;
	}

	if (!selity_limit_check($_POST['nreseller_max_mail_cnt'], -1)) {
		set_page_message(tr('Incorrect mail accounts limit!'));
		return false;
	}
	if (!selity_limit_check($_POST['nreseller_max_sql_db_cnt'], -1)) {
	...
	if (!selity_limit_check($_POST['nreseller_max_sql_user_cnt'], -1)) {
	...
	if (!selity_limit_check($_POST['nreseller_max_traffic'], null)) {
		set_page_message(tr('Incorrect traffic limit!'));
		return false;
	}
	if (!selity_limit_check($_POST['nreseller_max_disk'], null)) {
