...
$tpl->assign(
		array(
			'TR_ADMIN_I18N_PAGE_TITLE' => tr('Selity - Admin/Internationalisation'),
...
function install_lang() {
	...
	if (isset($_POST['uaction']) && $_POST['uaction'] === 'upload_language') {
		...
		if ($file_type !== "text/plain" && $file_type !== "application/octet-stream") {
			...
			$t = '';
			$ab = array('selity_languageSetlocaleValue' => '', 'selity_table' => '', 'selity_language' => '');
			$errors = 0;
			...
			if (empty($ab['selity_languageSetlocaleValue']) || empty($ab['selity_table']) || empty($ab['selity_language']) || !preg_match('/^[a-z]{2}(_[A-Z]{2}){0,1}$/Di', $ab['selity_languageSetlocaleValue']) || !preg_match('/^[a-z0-9]+$/Di', $ab['selity_table'])) {
				set_page_message(tr('Uploaded file does not contain the language information!'));
				return ;
			}

			$lang_table = 'lang_' . $ab['selity_table'];
			...
			if (!$lang_update) {
				write_log(sprintf("%s added new language: %s", $_SESSION['user_logged'], $ab['selity_language']));
				set_page_message(tr('New language installed!'));
			} else {
				write_log(sprintf("%s updated language: %s", $_SESSION['user_logged'], $ab['selity_language']));
