...
function gen_system_message(&$tpl, &$sql) {
	...
	$rs = exec_query($sql, $query, array($user_id, $user_id, 2));
...
function make_traff_usege($admin_id, $max_traff) {
	$sql = Database::getInstance();

	$fdofmnth = mktime(0, 0, 0, date("m"), 1, date("Y"));
	$ldofmnth = mktime(1, 0, 0, date("m") + 1, 0, date("Y"));
	$res = exec_query($sql,
		"SELECT IFNULL(sum(dtraff_web) + sum(dtraff_ftp) + sum(dtraff_mail) + sum(dtraff_pop),0) " . "as traffic FROM domain_traffic " . "WHERE admin_id=? and dtraff_time>? and dtraff_time<?",
		array($admin_id, $fdofmnth, $ldofmnth));
	$data = $res->FetchRow();
...
$props = get_user_default_props($_SESSION['user_id']);
	...
	$dmn_gid			= $props->user_gid;
	$dmn_uid			= $props->user_uid;
	$dmn_created_id		= $props->user_created_id;
	$dmn_created		= $props->user_created;
	$dmn_last_modified	= $props->user_last_modified;
	$dmn_mailacc_limit	= $props->max_mail;
	$dmn_ftpacc_limit	= $props->max_ftp;
	$dmn_traff_limit	= $props->max_traff;
	$dmn_sqld_limit		= $props->max_sqldb;
	$dmn_sqlu_limit		= $props->max_sqlu;
	$dmn_status			= $props->user_status;
	$dmn_als_limit		= $props->max_als;
	$dmn_subd_limit		= $props->max_sub;
	$dmn_ip_id			= $props->user_ips;
	$dmn_disk_limit		= $props->max_disk;
	$dmn_disk_usage		= $props->disk_usage;
	$dmn_php			= $props->php;
	$dmn_cgi			= $props->cgi;

$cnt =  get_user_running_props_cnt($_SESSION['user_id']);
	$sub_cnt		= $cnt['sub_cnt'];
	$als_cnt		= $cnt['als_cnt'];
	$mail_acc_cnt	= $cnt['mail_cnt'];
	$ftp_acc_cnt	= $cnt['ftp_cnt'];
	$sqld_acc_cnt	= $cnt['sqld_cnt'];
	$sqlu_acc_cnt	= $cnt['sqlu_cnt'];


$dtraff_pr = 0 ;
$dmn_traff_usege = 0;
$dmn_traff_limit = $dmn_traff_limit * 1024 * 1024;
list($dtraff_pr, $dmn_traff_usege) = make_traff_usege($_SESSION['user_id'], $props->max_traff);
...
$tpl->assign(array(
	'TR_PAGE_TITLE'	=> tr('Selity - Client/Main Index'),
