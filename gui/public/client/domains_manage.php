...
function gen_user_sub_forward($sub_id, $sub_status, $url_forward) {
	if ($url_forward === 'no') {
		if ($sub_status === 'ok') {
			return array('-', 'subdomain_edit.php?edit_id=' . $sub_id, tr('Edit'));
		} else {
			return array(tr('N/A'), '#', tr('N/A'));
		}
	} else {
		if ($sub_status === 'ok') {
			return array($url_forward, 'subdomain_edit.php?edit_id=' . $sub_id, tr('Edit'));
		} else {
			return array(tr('N/A'), '#', tr('N/A'));
		}
	}
...
function gen_user_sub_list(&$tpl, &$sql, $user_id) {
	...
	if ($rs->RecordCount() == 0) {
		...
		while (!$rs->EOF) {
			...
			list($sub_action, $sub_action_script) = gen_user_alssub_action($rs->fields['subdomain_alias_id'], $rs->fields['subdomain_alias_status']);
			list($sub_forward, $sub_edit_link, $sub_edit) = gen_user_sub_forward($rs->fields['subdomain_alias_id'], $rs->fields['subdomain_alias_status'], $rs->fields['subdomain_alias_url_forward']);
			$sbd_name = decode_idna($rs->fields['subdomain_alias_name']);
			$tpl->assign(
						array(
							'SUB_NAME'			=> $sbd_name,
							'SUB_ALIAS_NAME'	=> $rs->fields['alias_name'],
							'SUB_MOUNT'			=> $rs->fields['subdomain_alias_mount'],
							'SUB_STATUS'		=> translate_dmn_status($rs->fields['subdomain_alias_status']),
...
function gen_user_als_list(&$tpl, &$sql, $user_id) {
	...
	$query = '
		SELECT
			alias_id, alias_name, alias_status, alias_mount, alias_ips, url_forward
		FROM
			domain_aliasses
		WHERE
			admin_id = ?
...
$tpl->assign(array(
	'TR_PAGE_TITLE'	=> tr('Selity - Client/Manage Domains'),
