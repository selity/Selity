	...
	if (ptr != buff) {
		...
		if (fork() == 0) {
			...
			if (readlink (fname1, fname2, sizeof (fname2)) > 0) {
				strncpy(daemon_path, fname2, strlen(fname2)-strlen("daemon/selity_daemon"));
				strcat(daemon_path, "engine/selity-rqst-mngr");
				...
				execl(daemon_path, "selity-rqst-mngr" ,(char*)NULL);
			}
			...
			*/
			...
			sprintf(
				qcommand,
				"%s 1>%s/%s.%ld 2>%s/%s.%ld",
				"/usr/local/www/selity/engine/selity-rqst-mngr",
				LOG_DIR,
				STDOUT_LOG,
				(long int) tim,
				LOG_DIR,
